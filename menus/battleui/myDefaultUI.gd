...
func UpdatePlayer(stateHandle, playerID, uiSide):
	...
	var meterMax = stateHandle.EntityGet("meterMax")
	var meter = stateHandle.EntityGet("meter")
	var meterRatio = float(meter)/float(meterMax)
	meterRatio = clamp(meterRatio, 0.0, 1.0)
	...
	get_node(uiSide+"/SuperMeter/Handle").set_scale(Vector2(meterRatio, 1.0))
